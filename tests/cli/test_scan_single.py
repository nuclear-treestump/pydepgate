...
class FileKindAutodetectTests(unittest.TestCase):
    ...
    def test_sitecustomize_is_sitecustomize(self):
        result = _file_kind_for_single(
            pathlib.Path("/tmp/sitecustomize.py"),
            as_kind=None,
        )
    ...
    def test_usercustomize_is_usercustomize(self):
        result = _file_kind_for_single(
            pathlib.Path("/tmp/usercustomize.py"),
            as_kind=None,
        )
    ...
    def test_arbitrary_py_defaults_to_setup_py(self):
        ...
        result = _file_kind_for_single(
            pathlib.Path("/tmp/garbage_test.py"),
            as_kind=None,
        )
        self.assertIs(result, FileKind.SETUP_PY)
...
class FileKindOverrideTests(unittest.TestCase):
    ...
    def test_every_choice_has_a_mapping(self):
        ...
        for kind in ("setup_py", "init_py", "pth", "sitecustomize", "usercustomize"):
            result = _file_kind_for_single(
                pathlib.Path("/tmp/anything"),
                as_kind=kind,
            )
...
class DispatchSingleTests(unittest.TestCase):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for finding in result.findings:
                self.assertEqual(
                    ...
                    str(path),
                    msg=(
                        "finding context should reference the real "
    ...
    def test_pth_content_is_analyzed_as_pth(self):
        content = b"import base64; exec(base64.b64decode('cHJpbnQoMSk='))\n"
        with tempfile.TemporaryDirectory() as tmp:
...
class CliSingleFlagTests(unittest.TestCase):
    ...
    def test_single_flag_finds_density_signal_in_garbage(self):
        ...
        payload_str = ("QWxsIHRoZSB3b3JsZCdzIGEgc3RhZ2UsQWxsIHRoZSBz" * 4)[:128]
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn(rc, (0, 1, 2), msg=f"unexpected rc={rc}, stderr: {err}")
            payload = json.loads(out)
            ...
            self.assertGreater(
                len(density_ids), 0, msg=f"no density findings; got {ids}"
            )
    ...
    def test_single_event_log_records_hardened_completion_context(self):
        with tempfile.TemporaryDirectory() as tmp:
            path = pathlib.Path(tmp) / "snippet.py"
            path.write_bytes(b"x = 1\n")
            event_log = pathlib.Path(tmp) / "events.jsonl"
            rc, out, err = _run_cli(
                [
                    "scan",
                    "--single",
            ...
            self.assertEqual(completed["scan_mode"], "static.single")
            self.assertEqual(completed["target_kind"], "loose_file")
            self.assertEqual(completed["target_identity"], str(path))
            self.assertEqual(completed["target_ref"]["kind"], "loose_file")
            self.assertEqual(completed["result_kind"], "static_analysis")
            self.assertEqual(completed["artifact_identity"], str(path))
            run_completed = events[4]["payload"]
            self.assertEqual(run_completed["scan_mode"], "static.single")
            self.assertEqual(run_completed["target_kind"], "loose_file")
            self.assertEqual(run_completed["target_identity"], str(path))
            self.assertEqual(run_completed["target_ref"]["kind"], "loose_file")
            self.assertEqual(run_completed["result_kind"], "static_analysis")
            self.assertEqual(run_completed["artifact_identity"], str(path))
